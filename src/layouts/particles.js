...
var pJS = function (tag_id, params) {
  ...
  pJS.fn.particlesRefresh = function () {
    /* init all */
    window.cancelwindow.requestAnimFrame(pJS.fn.checkAnimFrame)
    window.cancelwindow.requestAnimFrame(pJS.fn.drawAnimFrame)
  ...
  pJS.fn.interact.linkParticles = function (p1, p2) {
    ...
    if (dist <= pJS.particles.line_linked.distance) {
      ...
      if (opacity_line > 0) {
        /*
          style
          https://github.com/marcbruederlin/particles.js/blob/master/src/particles.js
        */
        pJS.canvas.ctx.lineWidth = pJS.particles.line_linked.width

        let gradient = pJS.canvas.ctx.createLinearGradient(
          p1.x,
          p1.y,
          p2.x,
          p2.y
  ...
  pJS.fn.vendors.checkBeforeDraw = function () {
    ...
    if (pJS.particles.shape.type === "image") {
      if (pJS.tmp.img_type === "svg" && pJS.tmp.source_svg === undefined) {
        ...
        window.cancelwindow.requestAnimFrame(pJS.tmp.checkAnimFrame)
        if (!pJS.tmp.img_error) {
          pJS.fn.vendors.init()
          pJS.fn.vendors.draw()
        }
